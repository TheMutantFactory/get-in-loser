...
- **Whatever does not fit now lives under "More".** Eleven top level menus want about 470 pixels and a portrait phone offers 275, so the ones that do not fit collapse into a More item at the end of the bar — Image, Pixel, Voxel, Piano Roll, Layer, Effects, Tools and Help, each still opening its own submenu, one tap away instead of a swipe and a guess. It is a real menu item holding the overflowed menus as its children, which is why they nest, trigger and keyboard-navigate exactly as they did on the bar. Measured rather than guessed at, so it is not a phone feature: narrow a desktop window and the menus fold into More one at a time, and widen it and they walk back out.
- **And the bar still says when it is hiding something.** If the split has not been computed yet — or a browser manages to swallow both the resize event and ResizeObserver — the bar falls back to scrolling, with a fading edge and a chevron on whichever side still has menus, coming to rest on a whole menu name rather than slicing one under a hamburger. Opening or arrowing to a menu brings it clear of the hamburgers first, because they are painted over the bar's padding and "inside the scroll port" is not the same as "readable". All three decisions — how many fit, which edge hides something, how far to scroll to clear a gutter — are pure functions with twenty-six tests.
- **Fixed: the phantom scrollbar on submenu-heavy dropdowns.** The little ">" beside a menu that opens another is a stretched glyph, and the stretch counts towards the dropdown's scrollable height, so it spilled past the last row. Invisible until More arrived — the first dropdown where every single row has one and the content is exactly as tall as the box.
