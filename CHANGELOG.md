...
- **The bar says when there is more of it.** Eleven menus do not fit a phone, so the bar scrolls — and a scroll container is famously silent about what lies past its edge. It now carries a fading edge and a chevron on whichever side still has menus hidden, it comes to rest on a whole menu name instead of slicing one under a hamburger, and opening or arrowing to a menu brings it clear of the hamburgers rather than half under one. The two decisions behind that — which edge is hiding something, and how far to scroll to clear a gutter — are pure functions with fifteen tests, because the hamburgers overlay the bar's padding and "inside the scroll port" is therefore not the same as "readable".
